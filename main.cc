...
using halo_keyboard::FakeTouchpad;
using halo_keyboard::FakeKeyboard;
using halo_keyboard::TouchFFManager;

bool LoadHWConfig(std::string const &hw_config_file, struct halo_keyboard::hw_config &hw_config) {
  io::CSVReader<7,
    io::trim_chars<' ', '\t'>,
...
int main(int argc, char *argv[]) {
  struct halo_keyboard::hw_config hw_config;
  int debug_level = 0;
  int opt;
  ...
  while ((opt = getopt(argc, argv, "hdm:D:")) != -1) {
    switch (opt) {
      case 'h':
        std::cerr << "Usage: halo-keyboard-handler [-h] [-d] [-m <magnitude>] [-D <duration_ms>]\n";
        return 0;
      case 'd':
        debug_level++;
        break;
  ...
  LOG(INFO) << "Starting halo-keyboard-handler\n";

  if (!LoadHWConfig("hardware.csv", hw_config)) {
    LOG(ERROR) << "Failed to load hardware.csv\n";
    return EXIT_FAILURE;
  }
  ...
  try {
    if (pid < 0) {
      ...
      tp.Start(kTouchSensorDevicePath, "Halo Keyboard Touchpad");
    } else {
      ...
      kbd.Start(kTouchSensorDevicePath, "Halo Keyboard");
      wait(NULL);
    }
  } catch (...) {
    LOG(ERROR) << "Unhandled exception in Halo keyboard handler\n";
    exit(EXIT_FAILURE);
  }
