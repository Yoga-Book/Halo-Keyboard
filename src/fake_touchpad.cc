...
FakeTouchpad::FakeTouchpad(const HardwareConfig& hardware_config,
                           const std::filesystem::path& layout_path)
    : hardware_config_(hardware_config),
      slot_memberships_(multitouch::kNumSlots, false) {
  if (!LoadLayout(layout_path)) {
    throw std::runtime_error("touchpad configuration has no data row: " +
                             layout_path.string());
  }
}

bool FakeTouchpad::LoadLayout(const std::filesystem::path& layout_path) {
  ...
  if (!std::isfinite(xmin_mm) || !std::isfinite(ymin_mm) ||
      !std::isfinite(xmax_mm) || !std::isfinite(ymax_mm) ||
      xmin_mm >= xmax_mm || ymin_mm >= ymax_mm) {
    throw std::runtime_error("touchpad geometry is invalid");
  }
  ...
  switch (hardware_config_.rotation) {
    ...
    case 90:
      ...
      ymin_ = hardware_config_.height_mm - (left_margin + xmax_mm);
      ymax_ = hardware_config_.height_mm - (left_margin + xmin_mm);
      break;
    case 180:
      xmin_ = hardware_config_.width_mm - (xmax_mm + left_margin);
      xmax_ = hardware_config_.width_mm - (xmin_mm + left_margin);
      ymin_ = hardware_config_.height_mm - (ymax_mm + top_margin);
      ymax_ = hardware_config_.height_mm - (ymin_mm + top_margin);
      break;
    case 270:
      xmin_ = hardware_config_.width_mm - (ymax_mm + top_margin);
      xmax_ = hardware_config_.width_mm - (ymin_mm + top_margin);
      ymin_ = left_margin + xmin_mm;
      ymax_ = left_margin + xmax_mm;
      break;
    default:
      throw std::runtime_error("unsupported touchpad rotation");
  }
...
[[noreturn]] void FakeTouchpad::Run(const std::string& source_device_path,
                                    const std::string& touchpad_device_name) {
  if (!OpenSourceDevice(source_device_path)) {
    throw std::runtime_error("failed to open Halo touchpad input device");
  }
  if (!CreateUinputFD()) {
    throw std::runtime_error("failed to open uinput for touchpad");
  }

  // Enable the few button events that touchpads need.
  if (!EnableEventType(EV_KEY) || !EnableKeyEvent(BTN_TOUCH) ||
    ...
    throw std::runtime_error("failed to enable virtual touchpad buttons");
  }
  ...
  xres = std::lround(w / width_mm_);
  yres = std::lround(h / height_mm_);

  // Duplicate the ABS events from the source device.
  if (!CopyABSOutputEvents(source_fd_, w, h, xres, yres)) {
    throw std::runtime_error("failed to configure virtual touchpad axes");
  }

  // Finally, tell kernel to create the new fake touchpad's uinput device.
  if (!FinalizeUinputCreation(touchpad_device_name)) {
    throw std::runtime_error("failed to create virtual touchpad device");
  }
...
[[noreturn]] void FakeTouchpad::Consume() {
  while (1) {
...
bool FakeTouchpad::Contains(multitouch::Slot const &slot) const {
...
bool FakeTouchpad::PassEventsThrough(multitouch::Slot const &slot) const {
...
int FakeTouchpad::SyncTouchEvents() {
  ...
  for (std::size_t slot = 0; slot < multitouch::kNumSlots; ++slot) {
    const multitouch::Slot& source_slot = sm_.slot(slot);
    // Send a SLOT message to make sure these events go to the right slot
    SendEvent(EV_ABS, ABS_MT_SLOT, static_cast<int>(slot));

    // Don't pass on events from contacts outside of the region.
    if (!Contains(source_slot)) {
      ...
      if (!slot_memberships_[slot]) {
        int tid = source_slot.FindValueByEvent(EV_ABS, ABS_MT_TRACKING_ID);
    ...
    bool valid_finger = PassEventsThrough(source_slot);
